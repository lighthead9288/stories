...
class _StoriesWidgetState extends State<StoriesWidget> with TickerProviderStateMixin {
  ...
  late final Tween<Offset> _imageOffsetTween = Tween<Offset>(
    begin: Offset.zero,
    end: Offset.zero
  );
  late final Animation<Offset> _imageOffsetAnimation = _imageOffsetTween.animate(_imageOffsetController);
                        ...
                        children: [
                          Padding(
                            ...
                            child: ValueListenableBuilder(
                              valueListenable: _cubitContext.read<StoriesCubit>().storyPercentNotifier, 
                              builder: (_, double value, __) {
                                return Row(
                                  mainAxisAlignment: MainAxisAlignment.center,
                                  children: _getStoriesLinesPanelItems(storiesCount, value),
                                );
                              }
                            )
                          ),
                                ...
                                child: SlideTransition(
                                  ...
                                  child: Center(
                                    child: Container(
                                      ...
                                      child: Text('Image ${state.number + 1}', style: const TextStyle(color: Colors.white))
                                    ),
                                  ),
                                )
                              )
  ...
  List<Widget> _getStoriesLinesPanelItems(int storiesCount, double percent) {
    ...
    for(int i = 0; i < storiesCount; i++) {
      var progress = _cubitContext.read<StoriesCubit>().getStoryLoadingPercent(i, percent);
      lines.add(Container(
